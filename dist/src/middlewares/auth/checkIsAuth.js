...
const isLogin = async (req, res, next) => {
    ...
    jsonwebtoken_1.default.verify(token, configs_1.environmentConfig.ACCESS_TOKEN_SECRET_KEY, async (err, decodedUser) => {
        if (err) {
        ...
        try {
            const decodedUserInDB = await User_model_1.default.findOne({ _id: decodedUser?.userId }).select('-password -confirmPassword');
            if (!decodedUserInDB) {
...
exports.default = { isLogin: exports.isLogin };
